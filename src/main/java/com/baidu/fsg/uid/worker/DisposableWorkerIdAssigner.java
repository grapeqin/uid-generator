...
/**
 * Represents an implementation of {@link WorkerIdAssigner}, the worker id
...
public class DisposableWorkerIdAssigner implements WorkerIdAssigner {
	private static final Logger LOGGER =
			LoggerFactory.getLogger(DisposableWorkerIdAssigner.class);
	 ...
	 *
	 * <p>If there is host name in the environment, we considered that the
	 * node runs in Docker
	 * container<br>
	 ...
	 */
	@Override
	public synchronized long assignWorkerId() {
		String host = getHost();
		WorkerNodeEntity workerNodeEntity =
				workerNodeDAO.getWorkerNodeByHostPort(host);
	...
	private String getHost() {
		if (DockerUtils.isDocker()) {
			return DockerUtils.getDockerHost();
		}
		else {
